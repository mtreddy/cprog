...
ulong mem_test_data_lines(ulong addr)
{
    ...
    while(pattern <<= 1) {
        *tmp = pattern;
        ...
        l1_cache_line_flush_inv(ulong addr);
        if(*tmp != pattern)
            return pattern;
    }
...
ulong mem_test_addr_lines(ulong addr, int len)
{
    ...
    int offset = 0;
    int aoffset = 0;
    ulong addrmask = (len/(sizeof(int)) - 1);

    for(offset=1; (addrmask & offset) != 0; offset <<=1) {
        /*flush and invalidate cache*/
    ...
    *(tmp + aoffset) = apattern;
    for(offset = 1; (addrmask & aoffset) != 0; offset <<= 1) {
    ...
    *(tmp + aoffset) = pattern;
    ...
    for(aoffset = 1; (addrmask & aoffset) != 0; aoffset <<= 1) {
        *(tmp + aoffset) = apattern;
